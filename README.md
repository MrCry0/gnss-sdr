...
Install the library and headers (e.g., on Debian / Ubuntu, `sudo apt install
libbladerf-dev`, then check its version with `pkg-config --modversion
libbladeRF`; if it is older than 2.6.0, build `libbladeRF` from source instead
-- see [Nuand's bladeRF repository](https://github.com/Nuand/bladeRF)), then
configure GNSS-SDR to build the `Bladerf_Signal_Source` by:
...
single-channel (SISO) reception is currently supported (bladeRF 2.0 Micro
xA9's second RX channel is not yet exposed).
